# ВНИМАНИЕ !!! ПОКА СКРИПТ НЕ РАБОТАЕТ !!!  
# n8n-BEGET Install 🌌
...
```bash
bash <(curl -s https://raw.githubusercontent.com/DreamerBY/n8n-beget-install/main/install.sh)
```
...
```bash
docker image prune -f && docker system prune -f && truncate -s 0 /var/lib/docker/containers/*/*-json.log && find /tmp -type f -mtime +3 -delete
```
